...
class User(Base):
    ...
    cognito_user_id = Column(String, unique=True, index=True, nullable=False)  # Cognito 'sub' claim - ONLY unique identifier
    email = Column(String, nullable=True, index=True)  # Optional: may not be provided by some auth methods (social, magic link, phone)
    name = Column(String, nullable=True)  # Optional: from Cognito token (given_name/family_name)
