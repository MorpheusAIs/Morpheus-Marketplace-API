...
test_endpoints() {
    ...
    # Skip auth-demo endpoint test (not in original version)
    echo -e "${YELLOW}Skipping /auth-demo endpoint (not in original version)...${NC}"
    
    # Skip Cognito config endpoint test (not in original version)
    echo -e "${YELLOW}Skipping /api/v1/auth/cognito/config endpoint (not in original version)...${NC}"
